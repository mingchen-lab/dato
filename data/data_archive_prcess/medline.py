...
import re
# Yincong Zhou
# This script is modified from Bio.medline, to split addresses for the next step
...
def parse(handle):
    ...
    textkeys = ("ID", "PMID", "SO", "RF", "NI", "JC", "TA", "IS", "CY", "TT",
                ...
                "EDAT", "MHDA", "PST", "AB", "AD", "EA", "TI", "JT","MAIL")
    handle = iter(handle)
    count = 0
    record = Record()
    for key in textkeys:
    ...
    for line in handle:
        ...
        if line[:6] == "      ":  # continuation line
            ...
            if key == "AD":
                ...
                if(count>1):
                    record[key].append("@@@")
#                 record[key].append(re.sub('(\.)$',"",line[6:]).replace(".","***"))
                else:
                    record[key].append("######")
                record[key].append(line[6:])
            else:
